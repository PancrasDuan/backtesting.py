...
- 建立地图：[系统全景](#一张图建立系统全景) · [完整旅程](#一次回测的完整旅程)
- 七阶段精读：
...
- 综合串联：[四条调用链](#四条调用链速查) · [交易推演](#交易生命周期推演) · [不变量与记录模板](#架构不变量与源码阅读记录模板)
- 判断与验收：[项目边界](#项目边界与迁移启示) · [学习验收](#学习完成验收)
- 维护与速查：[Fork 与 upstream](#fork-与上游同步) · [源码索引与术语表](#附录源码索引与术语表)

## 七阶段依赖地图

顺读主干是“入口 → 时间 → 状态 → 撮合 → 结果”；阶段6还需要阶段2的数据传输概念和阶段5的结果边界。阶段7是贯穿式证据线：完成阶段1后即可与阶段2～6 **可并行** 阅读，每学完一种机制就去测试中确认；测试与手推不一致时，再按主题 **回看** 对应阶段，而不是继续向后堆概念。

```mermaid
flowchart LR
    S1["阶段1：公开 API"] --> S2["阶段2：时间推进"]
    S2 --> S3["阶段3：状态模型"]
    S3 --> S4["阶段4：撮合引擎"]
    S4 --> S5["阶段5：统计绘图"]
    S5 --> S6["阶段6：优化并行"]
    S1 -.->|"可提前开始"| S7["阶段7：测试规格"]
    S2 -.->|"行为证据"| S7
    S3 -.->|"状态证据"| S7
    S4 -.->|"成交证据"| S7
    S5 -.->|"结果证据"| S7
    S6 -.->|"并行证据"| S7
    S7 -.->|"失败时回看"| R["返回阶段2～6的对应机制"]
```

| 阶段 | 直接前置依赖 | 可并行与回看关系 |
|---|---|---|
| 阶段1 | 无；它建立公开入口和运行骨架 | 读完即可开始阶段7，先学会按行为找测试 |
| 阶段2 | 阶段1的 `Backtest.run` 主循环 | 未来数据或 warmup 断言不清时回看阶段1的装配顺序 |
| 阶段3 | 阶段1的策略 API；阶段2的逐 bar 时间 | 可与阶段2末尾的窗口练习交叉推演；对象状态混淆时回看所有权图 |
| 阶段4 | 阶段3的 Order/Trade/Position；阶段2的当前 OHLC | 与阶段7的最小行情测试并行；价格或 bar 不符时回看阶段2、3 |
| 阶段5 | 阶段4产生的 closed trades 与 equity；阶段1的收尾流程 | 与结果链速查交叉阅读；统计异常时回看阶段4的现金与成交 |
| 阶段6 | 阶段1的优化入口、阶段2的数据表示、阶段5的 `_Stats` 边界 | 可先略读进程图，再回看共享内存和 worker 返回值 |
| 阶段7 | 最低只需阶段1；完整验收依赖阶段2～6 | 贯穿其他阶段并行取证；任何失败都按数据、状态、撮合、统计、并行主题回看 |

速查时不必重走全部阶段：订单价格问题从阶段3→4，指标起点问题从阶段2→5，优化结果问题从阶段5→6；完成阶段4后可用“交易推演”串联状态与撮合，完成阶段5后用“四条调用链”复述全程，最后再进入边界判断与学习验收。
